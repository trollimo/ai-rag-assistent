...
Два компонента в одном репозитории — подробности и все параметры запуска в
их собственных README:

| Компонент | Назначение | README |
|-----------|------------|--------|
| `rag-generation/` | 🏗️ Индексация `.md` в ChromaDB + сборка устанавливаемых скиллов | [rag-generation/README.md](rag-generation/README.md) |
| `assistant-container/` | 🐳 Docker: Web UI + FastAPI + llama.cpp server + MCP | [assistant-container/README.md](assistant-container/README.md) |

### ✨ Фичи

- 🌐 **Web-форма** — человек задаёт вопрос → мини-модель → ответ из RAG
- 🤖 **MCP-интерфейс** — AI-агенты (opencode) получают знания через RAG
- 📦 **Устанавливаемые скиллы** — не только отвечаем словами, но и отдаём
  архив целиком для установки в code-агента
- 🔒 **Offline** — ни одного запроса в интернет при обработке знаний
- ⚙️ **Конфигурируемый** — гибкий `config` указывает папки с `.md` файлами

### Как это работает

![Как устроен продукт](docs/product-overview.svg)
...
### 🐳 Вариант через Docker (Streamable HTTP / SSE)

MCP сервер работает внутри контейнера на порту 9081. opencode подключается удалённо — не требует зависимостей на хосте.

**`.opencode.json`** (Streamable HTTP, рекомендуется):
```json
{
...
> Убедись, что в `docker-compose.yml` проброшен порт `9081:9081` и контейнер запущен.
