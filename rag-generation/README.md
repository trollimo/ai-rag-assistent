...
В одном файле — одна технология / одна тема. Пример:

```
docs/rules/
├── coding-rules.md       # общие правила
├── java-rules.md         # только Java
├── python-rules.md       # только Python
└── kotlin-rules.md       # только Kotlin
```

Почему это важно:
- чанки получаются тематически цельными;
- поиск находит релевантный файл по совпадению с запросом (Java → java-rules.md);
- LLM видит в тексте чанка название технологии и не смешивает правила.

### Пишите заголовки осмысленно

Первый заголовок `# ...` должен явно называть тему, например `# Python правила кодирования`, а не `# Правила`. Это повышает точность поиска.

Подробнее про чанкинг — в [документации ChromaDB](https://docs.trychroma.com/guides/build/chunking).
